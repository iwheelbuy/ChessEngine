...
public:
   ...
   uint8_t* map(void** baseAddress, uint64_t* mapping, const uint8_t* TB_MAGIC) {
      ...
      *baseAddress = mmap(nullptr, (size_t)statbuf.st_size, PROT_READ, MAP_SHARED, fd, 0);
      ::close(fd);
...
#ifndef _WIN32
      munmap(baseAddress, (size_t)mapping);
#else
...
uint8_t* set_sizes(PairsData* d, uint8_t* data) {
   ...
   d->sizeofBlock = size_t(1ULL << *data++);
   d->span = size_t(1ULL << *data++);
   ...
   for (int i = int(d->base64.size() - 2); i >= 0; --i) {
